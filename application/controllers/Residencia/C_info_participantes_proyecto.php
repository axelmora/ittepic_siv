...
class C_info_participantes_proyecto extends CI_Controller {
  ...
  public function asigna_revisores() {
    $asesor_revisor = array('id_docente' => $this->input->post('rfc'), 'tipo' => 'R');
    $ar = $this->m_info_participantes_proyecto->insertar_asesor_revisor($asesor_revisor);
    $tmp;
    foreach ($ar as $value) {
    ...
    if ($this->input->post('op')==1) {
      $this->m_info_participantes_proyecto->actualizar_participantes_proyecto($this->input->post('ppid'),array('revisor1'=>$tmp));
    }else {
      $this->m_info_participantes_proyecto->actualizar_participantes_proyecto($this->input->post('ppid'),array('revisor2'=>$tmp));
    }
    $this->participantes_proyecto();
  }
