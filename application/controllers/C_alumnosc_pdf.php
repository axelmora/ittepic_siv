...
class C_alumnosc_pdf extends CI_Controller {
    ...
    public function validar() {
        $this->form_validation->set_rules('carrerai', 'Carrera', 'required|min_length[1]|max_length[18]|trim');
        ...
        $this->form_validation->set_message('required', 'Debes seleccionar un campo');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $idcarrera = $this->input->post('carrerai');
            ...
            $result = $this->m_alumnosf_pdf->show_data_by_carrera($data);
            $results = $this->m_alumnosf_pdf->show_data_by_carrera($data);
            ...
            if ($result == 0) {
                $html .= "No hay alumnos de esta carrera haciendo su servicio social ";
            } else {
                $data['programa'] = $results;
                foreach ($data['programa'] as $it) {
                    $carrera = $it->carrera;
                }
                $html .= "<h2>Alumnos de la carrera " . $carrera . " con programa de servicio social registrado</h2><h4>Hay: " . count($result) . " Alumnos</h4>";
