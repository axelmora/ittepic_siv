...
<head>
  ...
  function confirmar()
  {
    if (confirm('¿Esta seleccionar este programa?'))
    return true;
    else
    return false;
  }
  ...
  <div id="primary">
    ...
    <ul class="collapsible" data-collapsible="expandable">
      <?php if ($noticias == null) { ?>
        <h3 align='center'> No hay noticias para mostrar </h3>
        <?php
      } else {
        foreach ($noticias as $item):
          ?>
          <li>
            <?php if ($item->fecha_noticia >= $dias) { ?>
              <div class="collapsible-header active  light-green accent-1 red-text darken-4-text"><i class="material-icons red-text">bookmark</i><?= $item->titulo_n; ?><div class="right"><font color="black">Fecha de publicación: </font><?= $item->fecha_noticia; ?></div></div>

            <?php } else { ?>
              <div class="collapsible-header active  grey lighten-4 amber-text "><i class="material-icons amber-text">bookmark_border</i><?= $item->titulo_n; ?><div class="right"><font color="black">Fecha de publicación: </font><?= $item->fecha_noticia; ?></div></div>

            <?php } ?>

            <div class="collapsible-body" ><br><p><?= $item->contenido_n; ?></p><br></div>
          </li>
          <?php
        endforeach;
      }
